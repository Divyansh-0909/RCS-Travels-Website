...
const Login = () => {
  ...
  // No <form> on native — the Button calls these directly, so there is no
  // event to cancel and no implicit submit to suppress.
  ...
  const verifyOtp = async () => {
    ...
    navigate(user.error ? "/signup" : "/home");
  };
  ...
  const boxStyle = (i) => {
    ...
    if (busy && i === 0) {
      return { backgroundColor: otpError ? BOX_FAIL : BOX_PASS, borderColor: "transparent" };
    }
    if (otpError) {
      return {
  ...
  return (
    ...
    >
      <Pressable
        ...
        aria-label="Back"
        onPress={back}
        className="absolute left-3 top-3 z-10 flex-row justify-center items-center p-2 active:opacity-60"
      >
      ...
      >
        {isSignedIn && !redirecting
          ? <View className="justify-center items-center">
            {/* font-semibold, not font-bold: tailwind.config maps bold to the
                Black cut here but the website's bold resolves to SemiBold. */}
                  ...
                  <View className="relative flex-row justify-center items-center gap-2">
                    {Array.from({ length: OTP_LENGTH }).map((_, i) => (
                      <TextInput
                        ...
                        selectTextOnFocus
                        className="font-sans text-2xl text-center w-[46px] h-[46px] my-1 rounded-xl border"
                        style={{ color: busy ? "transparent" : "#ffffff", ...boxStyle(i) }}
                      />
                    ))}
                    ...
                    {busy && (
                      <Animated.View
                        pointerEvents="none"
                        className="absolute left-0 right-0 top-0 bottom-0 z-20 items-center justify-center"
                      >
                        {error
