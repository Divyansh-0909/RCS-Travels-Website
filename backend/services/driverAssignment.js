...
export async function getDriver(bookingId) {
  ...
  const box = getBoundingBox(row.pickupLat, row.pickupLng, 10) 
  ...
  for (const x of sorted) {
    ...
    if (response === true ) {
      ...
      await prisma.booking.update({ where: { id: bookingId }, data: { status: 'assigned', driverId: x.driverId } })
