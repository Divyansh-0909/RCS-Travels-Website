...
import { dateFromText, parseBookingMessage, parseIncoming, parseScheduleTime, resolveLocation, timeFromText } from './whatsappParsing.js'
export { parseBookingMessage, parseIncoming, parseScheduleDate, parseScheduleTime, resolveLocation } from './whatsappParsing.js'
import { claimWhatsAppInbound } from './whatsappInbound.js'
export { messagesFromWebhook } from './whatsappInbound.js'
...
export async function processWhatsAppMessage(message) {
  if (!message?.id || !message?.from || !(await claimWhatsAppInbound(message))) return { duplicate: true }
  try {
