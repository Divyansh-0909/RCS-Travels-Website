    ...
    return (
        <div className="bg-[var(--foreground)] text-[var(--text-foreground)] gap-15 py-15 sm:py-20 flex flex-col items-center justify-center">
            <div className="text-left flex flex-col w-[82%] md:w-[90%] xl:w-[74%] justify-center gap-8 sm:gap-12 items-start">
                <h1 className="font-bold text-4xl sm:text-5xl">A ride for wherever you're headed</h1>
                <ul className="w-full flex flex-wrap items-start justify-left gap-10">
                    {data.map((item, index) => {
                        return (
                            <li key={index} className="w-full lg:w-[440px] xl:w-[542px] bg-[var(--foreground-muted)] rounded-xl p-6 sm:p-8 flex flex-col items-start justify-start gap-0 sm:gap-3 border-dashed">
                                <button
                                    type="button"
                                ...
                                >
                                    <h2 className="text-[25px] sm:text-2xl lg:text-3xl font-semibold">{item.title}</h2>
                                    <svg
                                ...
                                <div className={`grid w-full transition-[grid-template-rows] duration-300 ease-out sm:block ${openIndex === index ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                                    <div className="min-h-0 overflow-hidden sm:overflow-visible">
                                        <h3 className={`pt-3 sm:pt-0 sm:text-xl lg:text-2xl transition-opacity duration-300 ${openIndex === index ? "opacity-100" : "opacity-0"} sm:opacity-100`}>{item.description}</h3>
                                    </div>
                                </div>
                            </li>
                        )
