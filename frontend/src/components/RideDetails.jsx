...
const RideDetails = ({ prop }) => {
    ...
    async function handleCancel(e) {
        ...
        try {
            ...
            if (!cancelId) {
                prop.setError(dc("No active ride to cancel"));
                return;
            }
            ...
            if (data?.error) {
                ...
                prop.setError(dc("Can't cancel ride"));
                return;
            }
            if (data.ok) {
            ...
            console.error(err);
            prop.setError(dc("Something went wrong"));
        } finally {
    ...
    if (confirmCancel && canCancel) {
        return (
            ...
            >
                <button
                    type="button"
                    aria-label={dc("Back to ride details")}
                ...
                <div className={`flex flex-col gap-2 ${COL}`}>
                    <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">{tr("Ride cancellation")}</p>
                    <h2 id="cancel-ride-title" className="w-full min-w-0 [overflow-wrap:anywhere] text-3xl font-bold leading-tight tracking-[-0.03em] sm:text-5xl">{dc("Cancel this ride?")}</h2>
                    <p className="w-full min-w-0 text-base leading-snug text-[var(--text-muted)] sm:text-xl">{dc("Review what happens before you confirm.")}</p>
                </div>

                <div className={`${CARD} flex flex-col gap-3 ${COL}`}>
                    <div className="flex items-center justify-between gap-3">
                        <span className={FARE_LINE}>{tr("Cancellation charge")}</span>
                    ...
                    <p className="text-sm leading-snug text-[var(--text-muted)] sm:text-base">
                        {cancellationCharge > 0
                            ? dc("This comes from the advance you already paid and goes to your driver for reaching pickup. It is not a second charge.")
                            : dc("No cancellation charge will be added. If you paid an advance, its refund will be started after cancellation.")}
                    </p>
                </div>

                <div className={`flex flex-col items-stretch gap-2 ${COL}`}>
                    <Button onClick={handleCancel} prop={{ variant: "negative", width: "100%" }}>
                        <span className="text-base sm:text-lg">{tr("Yes, cancel ride")}</span>
                    </Button>
                    <Button
                        onClick={() => setConfirmCancel(false)}
                        prop={{ variant: "input", width: "100%", bg: "var(--background-muted)", border: false }}
                    >
                        <span className="text-base sm:text-lg">{tr("Keep ride")}</span>
                    </Button>
                </div>
            </div>
        );
    ...
    return (
        <div className="z-10 flex w-full flex-col items-center gap-4 text-left sm:order-1 sm:h-[100dvh] sm:w-auto sm:justify-center sm:overflow-y-auto sm:py-6">
            ...
            <div className={`flex items-center justify-start ${COL}`}>
                <h2 className="text-xl font-semibold leading-tight tracking-[-0.03em] sm:text-2xl">{tr("Ride Details")}</h2>
            </div>
            ...
            <div className={`flex flex-col gap-2 ${COL}`}>
                <p className="px-1 text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">{tr("Ride summary")}</p>
                <div className={`${CARD} flex flex-col gap-3`}>
                    <div className="flex flex-col gap-2">
                        <div className="flex items-start justify-between gap-3">
                            <span className={FARE_LINE}>{tr("Base fare")}</span>
                            <span className={FARE_LINE}>₹{baseFare}</span>
                        </div>
                        {safeRoute && (
                            <div className="flex items-start justify-between gap-3">
                                <span className={FARE_LINE}>{tr("Safer route")}</span>
                                <span className={FARE_LINE}>₹{surcharge}</span>
                            </div>
                        )}
                        {toll > 0 && (
                            <div className="flex items-start justify-between gap-3">
                                <span className={FARE_LINE}>{tr("Highway toll")}</span>
                                <span className={FARE_LINE}>₹{toll}</span>
                            </div>
                        )}
                        {airport > 0 && (
                            <div className="flex items-start justify-between gap-3">
                                <span className={FARE_LINE}>{tr("Airport pickup")}</span>
                                <span className={FARE_LINE}>₹{airport}</span>
                            </div>
                        )}
                        {carrier > 0 && (
                            <div className="flex items-start justify-between gap-3">
                                <span className={FARE_LINE}>{tr("Roof carrier")}</span>
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <span className="text-xl font-semibold">{tr("Total")}</span>
            ...
            <div className={`flex flex-col items-center gap-2 sm:items-start ${COL}`}>
                ...
                >
                    <span className="flex items-center justify-center gap-2 text-base sm:text-lg">
                        <img src={waLogo} alt="WhatsApp" className="h-6 w-6" />{dc("Talk to support")}</span>
                </Button>
                ...
                {canCancel && (
                    ...
                    >
                        <span className="text-base sm:text-lg">{tr("Cancel ride")}</span>
                    </Button>
                )}
            </div>
        </div>
